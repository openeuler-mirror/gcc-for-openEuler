...
export LD_LIBRARY_PATH=$PREFIX_NATIVE/lib:$PREFIX_NATIVE/lib64:$PREFIX_OPENSSL/lib64:$PREFIX_PUBLIC:$LD_LIBRARY_PATH
export CPLUS_INCLUDE_PATH=$PREFIX_NATIVE/include
export CC=$PREFIX_NATIVE/bin/gcc
export CXX=$PREFIX_NATIVE/bin/g++

cp $PREFIX_NATIVE/lib/libabsl* $PREFIX_NATIVE/lib/libproto*  $PREFIX_PUBLIC
...
cmake -G"Unix Makefiles" $ROOT_NATIVE_SRC/$GCC_CLIENT -DLLVM_DIR=$PREFIX_MLIR/lib/cmake/llvm -DMLIR_DIR=$PREFIX_MLIR/lib/cmake/mlir -DCMAKE_NO_SYSTEM_FROM_IMPORTED=1 -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$ROOT_NATIVE_DIR/obj/build-client -DCMAKE_PREFIX_PATH=$PREFIX_PUBLIC -DCMAKE_C_FLAGS="${SECURE_CFLAGS}" -DCMAKE_CXX_FLAGS="${SECURE_CFLAGS}" -DCMAKE_SHRAED_LINKER_FLAGS="${SECURE_LDFLAGS}"
make -j $PARALLEL && make install -j $PARAELLEL && popd
cp $ROOT_NATIVE_DIR/obj/build-client/lib/libpin_gcc_client.so $PREFIX_PUBLIC
...
sed -i "s@libdir = \$(prefix)/lib@libdir = \${PREFIX_PUBLIC}@" Makefile
make -j $PARALLEL && make install -j $PARAELLEL && popd
...
sed -i "s@LIBDIR := \$(DESTDIR)${PREFIX_NATIVE}/lib@LIBDIR := \$(DESTDIR)${PREFIX_PUBLIC}@" Makefile
...
cmake  -DCMAKE_INSTALL_PREFIX=$PREFIX_NATIVE -DCMAKE_INSTALL_LIBDIR=$PREFIX_NATIVE/lib64 -DCMAKE_BUILD_TYPE=Release -DYAML_CPP_BUILD_TOOLS:BOOL=OFF -DYAML_CPP_FORMAT_SOURCE:BOOL=OFF -DYAML_CPP_INSTALL:BOOL=ON -DYAML_BUILD_SHARED_LIBS:BOOL=ON -DYAML_CPP_BUILD_TESTS:BOOL=OFF
make -j $PARALLEL && make install -j $PARAELLEL && popd
...
cmake -DPython_EXECUTABLE=/usr/bin/python3.6 -DPython3_EXECUTABLE=/usr/bin/python3.6 -DCMAKE_INSTALL_PREFIX=$PREFIX_ONNXRUNTIME  -DCMAKE_INSTALL_LIBDIR=$PREFIX_ONNXRUNTIME/lib64 -DCMAKE_INSTALL_INCLUDEDIR=$PREFIX_ONNXRUNTIME/include -Donnxruntime_BUILD_SHARED_LIB=ON -Donnxruntime_BUILD_UNIT_TESTS=OFF -Donnxruntime_INSTALL_UNIT_TESTS=OFF -Donnxruntime_BUILD_BENCHMARKS=OFF -Donnxruntime_USE_FULL_PROTOBUF=ON -Donnxruntime_DISABLE_ABSEIL=ON -DCMAKE_BUILD_TYPE=Release -S cmake
make -j $PARALLEL && make install -j $PARAELLEL && popd
cp -r $PREFIX_ONNXRUNTIME/lib64/* $PREFIX_PUBLIC
cp -r $PREFIX_ONNXRUNTIME/include/onnxruntime/* $PREFIX_NATIVE/include

# Install aiframe required by AI4C.
echo "Building aiframe for AI4C..." && pushd $ROOT_NATIVE_DIR/obj/build-AI4C
cp ./models/* $PREFIX_PUBLIC/AI4C
cd ./python
python3 setup.py bdist_wheel -Donnxruntime_ROOTDIR=$PREFIX_ONNXRUNTIME -DCMAKE_BUILD_TYPE=RelWithDebInfo -Dcrypto_LIBDIR=$PREFIX_PUBLIC 
cd ./dist
cp *.whl $PREFIX_PUBLIC/AI4C
popd
...
mv $PREFIX_NATIVE/lib64/libyaml* $PREFIX_NATIVE/lib/libabsl* $PREFIX_NATIVE/lib/libproto* $PREFIX_PUBLIC
...
pushd $PREFIX_NATIVE/.. && chmod 755 $INSTALL_NATIVE -R
